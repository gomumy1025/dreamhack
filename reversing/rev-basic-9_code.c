//Wargame site URL: https://dreamhack.io/wargame/challenges/23
...
int unk_140004000[] =
{
  126, 125, 154, 139,  37,  45, 213,  61,   3,  43,
   56, 152,  39, 159,  79, 188,  42, 121,   0, 125,
  196,  42,  79,  88,   0,   0,   0,   0,   0,   0,
    0,   0
};
...
int main(void) {
    ...
    for (int r = 0; r < 3; r++) {
        int un[10];
        for (int idx = r * 8; idx < (r + 1) * 8; idx++) {
            un[idx % 8] = unk_140004000[idx];
        }
        for (int i = 15; i >= 0; --i) {
            for (int j = 7; j >= 0; --j) {
                int v2 = un[(j + 1) & 7] & 255;
                un[(j + 1) & 7] = (ROL(v2, 5) - byte_140004020[v5[j] ^ un[j & 7]]) & 255;
            }
        }
        for (int k = r * 8; k < (r + 1) * 8; k++) {
            flag[k] = un[k % 8];
        }
    }

    printf("Completed. Result is ");
    for (int i = 0; i < 24; i++)printf("%c", flag[i]);
    printf("\n\n");
